...
int main(){
    ...
    int codigo,horaEntrada,minEntrada,horaSalida,minSalida;
...
// de acuerdo a las diferentes horarios posibles considera retraso
void penalizacion(int codigo,int horaEntrada,int minEntrada,int horaSalida,int minSalida){
    if((horaEntrada==8&&minEntrada<=10)||(horaEntrada<8&&minEntrada<60)){       // tolerancia y horarios fuera del trabajo
        if(horaSalida==16&&minSalida>=0){
            cout<<"Penalizacion: "<<0;
        }
    }
    else{
        if(horaEntrada==8&&minEntrada>10){      // salida
            if(horaSalida<16&&minSalida<60){
                cout<<"Penalizacion: "<<minEntrada+(60-minSalida);
            }
        }
        else{
            if(horaEntrada<8&&minEntrada<60){       // salida antes del horario
                if(horaSalida<16&&minSalida<60){
                    cout<<"Penalizacion: "<<(60-minSalida);
                }
            }
        }
    }
